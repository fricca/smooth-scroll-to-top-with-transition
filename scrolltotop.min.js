...
}else{d.innerText=s.topLinkContent;}h.appendChild(d);return d;}function z(w){var A,B,d;for(A=0,B=w.length;A<B;A++){x.addClass(w[A],y.linkClass);x.addEvent(w[A],"click",c);
}if(w.length===1){x.addClass(w[0],y.singleClass);if(s.hiddenDistance>0){d=w[0];x.addClass(w[0],y.hiddenClass);}else{x.addClass(w[0],y.visibleClass);}}if(d||x.test.transitionEnd){x.addEvent(g,"scroll",function(){o(d);
});}}function p(d){var w;if(d){u(d);}if(s.linkName){w=l(s.linkName);if(w.hasOwnProperty("className")){r=x.getElementsByClass(w.className);}else{if(w.hasOwnProperty("id")){r[0]=t.getElementById(w.id);
}}if(!r[0]){return;}}else{r[0]=m();}if(r.length>0){z(r);}}return p;}(window,document));